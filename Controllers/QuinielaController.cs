using PresentacionQuinielaGuaymura.Models;
...
namespace QuinielaGuaymura.Controllers
{
    public class QuinielaController : Controller
    {
        ...
        public IActionResult RealizarQuiniela()
        {
            ...
            string query = @"
                SELECT 
                    p.ID_Partido, p.Grupo, p.Fecha_Partido,
            ...
            return View("RealizarQuiniela", partidosPorGrupo);
        }

        // ── Guardar Quiniela POST ─────────────────────────────────────
        [HttpPost]
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    foreach (string key in form.Keys)
                    {
                        ...
                        using (SqlCommand cmdCheck = new SqlCommand(
                            "SELECT COUNT(*) FROM PREDICCIONES WHERE ID_Usuario = @u AND ID_Partido = @p", connection))
                        {
                            cmdCheck.Parameters.AddWithValue("@u", idUsuario.Value);
                        ...
                        if (existe == 0)
                        {
                            ...
                            {
                                cmdInsert.Parameters.AddWithValue("@u", idUsuario.Value);
                        ...
                        else
                        {
                                  ...
                                  WHERE ID_Usuario = @u AND ID_Partido = @p", connection))
                            {
                                ...
                                cmdUpdate.Parameters.AddWithValue("@u", idUsuario.Value);
        ...
        // ── Ver Quiniela GET — solo lectura ───────────────────────────
        public IActionResult VerQuiniela()
        {
            string usuarioActivo = HttpContext.Session.GetString("UsuarioActivo");
            if (string.IsNullOrEmpty(usuarioActivo))
            {
                TempData["Mensaje"] = "⚠️ Debes iniciar sesión para ver tu quiniela.";
                TempData["TipoMensaje"] = "warning";
                return RedirectToAction("Index", "Home");
            }
            ...
            // Trae los partidos con la predicción del usuario
            string query = @"
                SELECT 
                    pt.ID_Partido,
                    pt.Grupo,
                    ...
                    pr.Puntos_Ganados
                FROM partidos pt
                JOIN paises pa1 ON pt.ID_Pais_Local     = pa1.ID_Pais
                JOIN paises pa2 ON pt.ID_Pais_Visitante = pa2.ID_Pais
                LEFT JOIN PREDICCIONES pr ON pt.ID_Partido = pr.ID_Partido 
                                          AND pr.ID_Usuario = @idUsuario
                ORDER BY pt.Grupo, pt.Fecha_Partido";

            var partidosPorGrupo = new Dictionary<string, List<PrediccionViewModel>>();
                ...
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            string grupo = reader["Grupo"].ToString().Trim();
                            if (!partidosPorGrupo.ContainsKey(grupo))
                                partidosPorGrupo[grupo] = new List<PrediccionViewModel>();

                            partidosPorGrupo[grupo].Add(new PrediccionViewModel
                            {
                                IdPartido = Convert.ToInt32(reader["ID_Partido"]),
                                Grupo = grupo,
                                FechaPartido = Convert.ToDateTime(reader["Fecha_Partido"]),
                                PaisLocal = reader["PaisLocal"].ToString().Trim(),
                                PaisVisitante = reader["PaisVisitante"].ToString().Trim(),
                                GolesLocalPrediccion = reader["Goles_Local_Prediccion"] == DBNull.Value ? (int?)null : Convert.ToInt32(reader["Goles_Local_Prediccion"]),
                                GolesVisitantePrediccion = reader["Goles_Visitante_Prediccion"] == DBNull.Value ? (int?)null : Convert.ToInt32(reader["Goles_Visitante_Prediccion"]),
                                PuntosGanados = reader["Puntos_Ganados"] == DBNull.Value ? (int?)null : Convert.ToInt32(reader["Puntos_Ganados"])
                            });
                        }
                    }
                }
            ...
            {
                TempData["Mensaje"] = "❌ Error al cargar tu quiniela.";
                TempData["TipoMensaje"] = "danger";
                return RedirectToAction("Index", "Home");
            }

            ViewBag.NombreUsuario = usuarioActivo;
            return View("VerQuiniela", partidosPorGrupo);
        }
    }
}